...
class PMMTrendV1ControllerConfig(MarketMakingControllerConfigBase):
    controller_name: str = "pmm_trend_v1"
...
class PMMTrendV1Controller(MarketMakingControllerBase):
    def __init__(self, config: PMMTrendV1ControllerConfig, *args, **kwargs):
        # for candles config
    ...
    def determine_trend(self, candles: pd.DataFrame, last_candle_timestamp: int) -> Tuple[str, float]:
        ...
        ao = pta.ao(candles_high, candles_low)
    ...
    async def update_processed_data(self):
        ...
        candles = self.market_data_provider.get_candles_df(connector_name=self.config.connector_name, trading_pair=self.config.trading_pair, 
                                                           interval=self.config.candle_interval, max_records=self.max_records)
        ...
        (trend, candle_natr) = self.determine_trend(candles, last_candle_timestamp)
    ...
    def get_executor_config(self, level_id: str, price: Decimal, amount: Decimal):
        ...
        trailing_stop_activation_price = abs(initial_config.trailing_stop.activation_price * take_profit)
        trailing_stop_delta = abs(initial_config.trailing_stop.trailing_delta * trailing_stop_activation_price)
