...
class PMMTrendingAdaptiveV7ControllerConfig(MarketMakingControllerConfigBase):
    controller_name: str = "pmm_trending_adaptive_v7"
    ...
    early_stop_activation_price_min: float = Field(
        default=0.008,
        json_schema_extra={"prompt": "Enter the min early_stop_activation_price(0.008): ", "prompt_on_new": True, "is_updatable": True})
    early_stop_activation_multiplier_min: float = Field(
        default=2,
        json_schema_extra={"prompt": "Enter the min early_stop_activation_multiplier(2): ", "prompt_on_new": True, "is_updatable": True})
...
class PMMTrendingAdaptiveV7Controller(MarketMakingControllerBase):
    def __init__(self, config: PMMTrendingAdaptiveV7ControllerConfig, *args, **kwargs):
        # for candles config
    ...
    async def update_processed_data(self):
        ...
        if abs(candle_cci) > self.config.cci_threshold:
            trend = 'high_cci'
    ...
    def determine_early_stop(self):
        ...
        for executor_info in self.executors_info:
            ...
            # take_profit_pct = executor_info.config.triple_barrier_config.take_profit
            # estimated_natr = take_profit_pct / self.config.take_profit
            # min_early_stop_price = estimated_natr * Decimal(self.config.early_stop_activation_multiplier_min)
            # early_take_profit = max(take_profit_pct / Decimal(time_factor), min_early_stop_price)
            ...
            if executor_info.net_pnl_pct > early_take_profit:
                executors_to_early_stop.append(executor_info)
                # self.logger().warning(f'Add {executor_info} to early take profit, pnl:{executor_info.net_pnl_pct:.4%}, threshold:{early_take_profit:.4%}(take_profit_pct:{take_profit_pct:.4%}, min_early_stop_price:{min_early_stop_price:.4%}, estimated_natr:{estimated_natr:.4%}, multiplier:{self.config.early_stop_activation_multiplier_min:.1f}), execution_time:{execution_seconds}s')
                self.log_msg(f'Add {executor_info} to early take profit, pnl:{executor_info.net_pnl_pct:.4%}, threshold:{early_take_profit:.4%}, execution_time:{execution_seconds}s')
